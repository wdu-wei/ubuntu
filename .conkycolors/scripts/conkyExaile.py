...
#    23/09/2009    Updated to work with new Exaile dbus service
#    23/09/2009    Added --maxlength option already available with Rhythmbox and Banshee scripts
            ...
            if self.musicData == None:
                ...
                if self.testDBus(bus, 'org.exaile.Exaile'):
                    ...
                    try:
                        ...
                        coverart = ""
                        try:
                            coverart = iface.GetCoverData() # still not working, dbus errors!
                        except:
                            coverart = ""
                        
                        # if cover art found then use it, otherwise use default coverart image for this plugin
                        if len(coverart) > 0 and coverart.find("noexailecover.png") == -1:
                            coverart = coverart.encode("utf-8")
                            
                            #handle escape chars, are there anymore??? Is there a better way to do this?
                        ...
                        if self.isNumeric(length) == False:
                            length = 0
                        else:
                            length = float(length)
...
def main():
    ...
    if options.version == True:
        print >> sys.stdout,"conkyExaile v.2.09"
    else:
